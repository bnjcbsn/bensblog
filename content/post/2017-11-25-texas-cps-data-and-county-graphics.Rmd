...
Now I have lat and long associated with the county name. How well does this plot in it's initial state? The initial code here is sourced from Chen [@chen_2017]. 
```{r plot4}
ggplot() + 
    geom_polygon(data = dfsp3, aes(x = long, y = lat, group = group), colour = "black", fill = NA)
```

My research diverges to a review of CRAN at the [CRAN Task View: Analysis of Spatial Data](https://cran.r-project.org/view=Spatial). This led me to a recent paper by Robin Lovelace and James Cheshire found in the task view fourth paragraph, which further led me to Robin Lovelace's [Github repo](https://github.com/Robinlovelace/Creating-maps-in-R) for the paper. 
...
I strongly suspect that if we plotted population, we'd see extremely similar patterns.
